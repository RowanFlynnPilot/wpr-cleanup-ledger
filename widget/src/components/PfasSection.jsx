...
export default function PfasSection({ systems, error, loading, selected, onSelect }) {
  return (
    <section className="pfas" aria-label={PFAS_COPY.title}>
      ...
      <p className="pfas__dek">{PFAS_COPY.dek}</p>
      <p className="pfas__caveat">{PFAS_COPY.caveat}</p>
      {error ? (
